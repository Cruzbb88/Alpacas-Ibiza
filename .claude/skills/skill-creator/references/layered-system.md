...
### Cortex Integration per Layer

Each layer has a natural CLI/MCP split. See `references/cortex-cli-patterns.md` for full command reference and anti-patterns.

- **L1 (Analysis):** MCP `cortex_recall` to discover prior context for decision-making. CLI `cortex remember` to store findings.
- **L2 (Patterns):** MCP `cortex_list_memories` to present options to user. CLI `cortex remember` to store decisions and pattern observations.
- **L3 (Deep/Execution):** CLI only — `cortex remember` results, `cortex link` relationships, `cortex log-activity` progress. No MCP needed; execution layers are fire-and-forget.
- **L4 (Trends):** CLI `cortex recall --json | jq` to verify stored data and compare historical scores. CLI `cortex export` for data aggregation. MCP only if user needs interactive verification results.
- **Reporting layers:** CLI `cortex remember` to store report metadata. CLI `cortex export` for cross-report data.
- **Handoff layers:** CLI `cortex remember` for session/handoff summaries. MCP `cortex_recall` only for final summary retrieval when the LLM needs it in-context.

**Rule of thumb:** If the layer produces data the LLM reasons about interactively, use MCP. If the layer stores/logs data and moves on, use CLI.
