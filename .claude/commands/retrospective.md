...
SESSION_NAME: $1
RETRO_DIR: reports/retrospectives
TIMESTAMP: Current date in YYYY-MM-DD format

## Workflow

1. **Initialize Retrospective**
   - Create `<RETRO_DIR>` directory if it doesn't exist (`mkdir -p reports/retrospectives/`)
   - **Sequential File Naming** (unified report convention, see `~/.claude/skills/REPORT-CONVENTION.md`):
     1. Get today's date in YYYY-MM-DD format
     2. Glob `reports/retrospectives/retro-*.md`, extract highest NNN after the prefix
     3. Increment to get next number (start at 001 if none exist)
     4. Generate description slug from SESSION_NAME or primary task context (kebab-case, max 50 chars)
     5. Generate filename: `retro-{NNN}-{YYYY-MM-DD}-{slug}.md`
        - Example: `retro-001-2026-01-11-adw-migration.md`
        - Example: `retro-002-2026-01-11-weekend-sprint-review.md`
...
5. **Include Self-Heal Trend Data**
   If `reports/self-heal/` exists with 2+ `sh-*.md` reports:
...
6. **Generate Retrospective Document** (includes Self-Heal Trends from step 5 if available)
   - Create markdown file in `<RETRO_DIR>` with YAML frontmatter followed by content.
   - **YAML Frontmatter** (required, see `~/.claude/skills/REPORT-CONVENTION.md`):
     ```yaml
     ---
...
### Files Created
- `reports/retrospectives/retro-NNN-YYYY-MM-DD-{slug}.md`
