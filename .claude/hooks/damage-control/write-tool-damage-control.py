...
def match_path(file_path: str, pattern: str) -> bool:
    ...
    if is_glob_pattern(pattern):
        ...
        # For filename patterns (start with . or no path separator), do basename matching
        if pattern.startswith('.') or (os.sep not in pattern and '/' not in pattern):
...
def check_path(file_path: str, config: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    # Check allowed paths first - these override zero-access patterns
    # Used for safe template files like .env.example
    for allowed_path in config.get("allowedPaths", []):
        if match_path(file_path, allowed_path):
            return False, ""  # Explicitly allowed, skip all blocking
