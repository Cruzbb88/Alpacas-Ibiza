...
def command_mentions_allowed_path(command: str, allowed_paths: List[str]) -> bool:
    """Check if command explicitly mentions an allowed path (exception to zero-access).

    This allows operations on safe template files like .env.example even though
    they match broader patterns like .env.*
    """
    for allowed in allowed_paths:
        if is_glob_pattern(allowed):
            glob_regex = glob_to_regex(allowed)
            try:
            ...
            if re.search(escaped, command, re.IGNORECASE):
                return True
    return False



def is_zero_access_operation(command: str, path: str) -> bool:
    """Check if command performs an actual file operation on a zero-access path.
    
    This is smarter than just checking if the path appears in the command.
    It only blocks actual file operations, not just mentions of the path as a string.
    
    For example:
    - "cat secrets" -> blocked (reading file)
    - "echo x > secrets" -> blocked (writing file)  
    - 'cat > file.py << EOF path = "secrets" EOF' -> ALLOWED (just a string in code)
    """
    # Build pattern to match the path (handle both glob and literal)
    if is_glob_pattern(path):
        path_regex = glob_to_regex(path)
    else:
        expanded = os.path.expanduser(path)
        # Match either the original or expanded form
        path_regex = f"({re.escape(path)}|{re.escape(expanded)})"
    
    # File reading operations that should be blocked
    read_ops = [
        rf'\bcat\s+[^|><]*{path_regex}',        # cat file
        rf'\bless\s+[^|><]*{path_regex}',       # less file
        rf'\bmore\s+[^|><]*{path_regex}',       # more file
        rf'\bhead\s+[^|><]*{path_regex}',       # head file
        rf'\btail\s+[^|><]*{path_regex}',       # tail file
        rf'\bgrep\s+[^|]*{path_regex}',         # grep pattern file
        rf'\bawk\s+[^|]*{path_regex}',          # awk ... file
        rf'\bsed\s+[^|]*{path_regex}',          # sed ... file
        rf'\bsource\s+{path_regex}',            # source file
        rf'\.\s+{path_regex}',                  # . file (source shorthand)
        rf'\bcp\s+{path_regex}',                # cp file ...
        rf'\bmv\s+{path_regex}',                # mv file ...
    ]
    
    # File writing operations that should be blocked
    write_ops = [
        rf'>\s*{path_regex}',                   # > file
        rf'>>\s*{path_regex}',                  # >> file
        rf'\btee\s+[^|]*{path_regex}',          # tee file
        rf'\bcp\s+[^|]+\s+{path_regex}',        # cp something file
        rf'\bmv\s+[^|]+\s+{path_regex}',        # mv something file
        rf'\btouch\s+{path_regex}',             # touch file
    ]
    
    # Delete operations that should be blocked
    delete_ops = [
        rf'\brm\s+[^|]*{path_regex}',           # rm file
        rf'\bunlink\s+{path_regex}',            # unlink file
        rf'\bshred\s+[^|]*{path_regex}',        # shred file
    ]
    
    all_ops = read_ops + write_ops + delete_ops
    
    for pattern in all_ops:
        try:
...
def check_command(command: str, config: Dict[str, Any]) -> Tuple[bool, bool, str]:
    ...
    # 2. Check for file operations on zero-access paths
    #    Uses smarter detection - only blocks actual file ops, not string mentions
    #    Skip if command uses an explicitly allowed path (e.g., template files)
    if not command_mentions_allowed_path(command, allowed_paths):
        for zero_path in zero_access_paths:
            if is_zero_access_operation(command, zero_path):
