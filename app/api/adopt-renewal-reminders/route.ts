...
export async function GET(request: Request) {
    return runCron(request, {
        routeName: 'adopt-renewal-reminders',
    }, async () => {
        const log = makeRequestLogger('adopt-renewal-reminders', 'runner')
        ...
        // ── Stripe path ───────────────────────────────────────────────────────
        // Hoist resolved clients so the per-recipient impact-data block can reuse
        // them without calling importStripe() / getMollieClient() again per recipient.
        ...
        // ── Mollie path ───────────────────────────────────────────────────────
        let mollieClient: import('@mollie/api-client').MollieClient | null = null
        ...
        const tasks = recipients.map(async (r) => {
            ...
            try {
                if (r.vendor === 'stripe' && stripeClient) {
                    // Reuse the already-resolved Stripe client (no re-import per recipient)
                ...
                } else if (r.vendor === 'mollie' && mollieClient) {
                    // Reuse the already-resolved Mollie client (no re-init per recipient)
                    const paymentsPage = await (mollieClient as unknown as {
                        payments: {
                            list: (opts: {
                                customerId: string
                                limit: number
