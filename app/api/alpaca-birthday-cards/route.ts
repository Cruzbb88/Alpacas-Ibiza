...
export async function GET(request: Request) {
    return handleBirthdayCards(request)
}
...
async function handleBirthdayCards(request: Request) {
    const reqId = getRequestId(request)
    const log = makeRequestLogger('alpaca-birthday-cards', reqId)
    ...
    const tasks = recipients.map(async (r): Promise<SendResult> => {
        const billingPortalUrl = `${SITE_BASE_URL}/${r.locale}/adopt#manage`
