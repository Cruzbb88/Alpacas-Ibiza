...
export async function GET(request: Request) {
    return runCron(request, {
        routeName: 'owner-mrr-digest',
        heartbeatEnvKey: 'HEARTBEAT_OWNER_MRR_DIGEST_URL',
    }, async (): Promise<CronRunResult<unknown>> => {
        const log = makeRequestLogger('owner-mrr-digest', 'runner')

        // ── Mollie API key gate ───────────────────────────────────────────────
        const apiKey = process.env.MOLLIE_API_KEY
        if (!apiKey) {
            log.warn('MOLLIE_API_KEY unset — cannot compute MRR')
            // 5xx here intentional: this is a config error, not a business no-op.
            // Throwing causes runCron to return 500 so Vercel retries.
            throw new Error('MOLLIE_API_KEY_UNSET')
        }
        ...
        if (isAlreadyProcessed(runKey)) {
            ...
            return {
                ...
                failures: 0,
                skipped: 1,
                detail: { sent: false, idempotent: true, thisMonday },
        ...
        try {
            ...
            // Fail-quiet: log but return success so Vercel cron doesn't retry.
            // A retry would re-send the same digest — worse than a missed send.
            log.error('sendEmail failed', { err: String(err) })
            // Heartbeat is fired by runCron after this runner returns, so the
            // dead-man's switch is still pinged. The send-fail is surfaced via
            // log.error for ops review.
            return {
                ok: true,
                routeName: 'owner-mrr-digest',
                durationMs: 0,
                ...
                failures: 1,
                skipped: 0,
                detail: { sent: false, reason: 'send-failed', mrr, activeCount },
            } satisfies CronRunResult
        }
        ...
        return {
            ...
            failures: 0,
            skipped: 0,
            detail: { sent: true, mrr, activeCount },
        } satisfies CronRunResult
    })
}
