...
| `lib/owner-notify.ts` fans escalation transitions to Slack / Telegram / generic webhook (fail-quiet, 5s timeout) | [lib/owner-notify.ts](lib/owner-notify.ts) | only fires on `at-risk` / `action-required` (skips `first`); env vars are optional; throws are swallowed so webhook 200 contract is preserved |
