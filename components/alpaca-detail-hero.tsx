...
export async function AlpacaDetailHero({ locale, animal }: AlpacaDetailHeroProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
      ...
      {hasImage ? (
        ...
        <div
          role="img"
          aria-label={`${translate('alpacas.photoComingSoon')}: ${animal.name}`}
      ...
      <div className="flex flex-col gap-5">
        ...
        {(animal.breed || animal.age != null || animal.color) && (
          <div className="flex flex-wrap gap-2">
            ...
            {animal.age != null && (
              <span className="inline-block rounded-full bg-secondary/80 px-3 py-1 text-xs font-semibold text-foreground/80">
                {animal.age} {translate('alpacas.yearsOld')}
              </span>
            )}
            {animal.color && (
              <span className="inline-block rounded-full bg-secondary/80 px-3 py-1 text-xs font-semibold text-foreground/80">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-2">
          <Link
            ...
            aria-label={`${translate('alpacas.adoptCta')} ${animal.name}`}
          >
            {translate('alpacas.adoptCta')} {animal.name} →
          </Link>
          <Link
            href={listHref}
